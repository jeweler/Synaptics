...
class Model
{
    var $con, $table, $error, $rows, $result = array(), $querystack = array(), $insert;

    function __construct($table)
    {
        if (class_exists('YAML')) {
            $conn = YAML::YAMLLoad(DIR . '/core/configs/config.yaml');
    ...
    {
        if (mysql_errno())
            new Except(new Exception(mysql_error($this->con)));
    }

    private function haveRow($row)
    {
    ...
    function find($rows = false, $where = false, $join = false, $order = false, $limit = false)
    {
        ...
        if ($join) {
            if (is_array($join)) {
                if (count($join) == 3) {
                    $query = array_merge($query, array('LEFT JOIN', $join[0], 'ON', $join[0] . '.' . $join[1], '=', $this->table . '.' . $join[2]));
                }
            }
        }
        if ($where) {
        ...
        if ($order) {
            if (is_array($order)) {
                if (count($order) == 2) {
                    if ($this->haveRow($order[0])) $order[0] = '`' . $order[0] . '`';
    ...
    function removeByParam($param, $value)
    {
        $this->find(false, array($param => $value));
        $this->delete();
    }
    ...
    function add($params)
    {
        if (count($params) > 0) {
            ...
            if (count($_s) > 0) {
                ...
                $this->insert = mysql_insert_id($this->con);
    ...
    function getByQuery($query)
    {
        ...
        if ($query) {
            if (mysql_num_rows($query) > 0) {
                while ($result = mysql_fetch_array($query)) {
                    foreach ($result as $key => $value) {
                        $result[$key] = (json_decode($value) == null) ? is_null($value) ? '' : $value : json_decode($value);
                    }
            ...
            new Except(new Exception('Ошибка запроса sql:' . $req . '<br>' . mysql_errno($this->con) . '::' . mysql_error($this->con)));
            $this->error = mysql_errno($this->con) . '::' . mysql_error($this->con);
            $this->result = null;
            $this->query = $query;
            return array();
