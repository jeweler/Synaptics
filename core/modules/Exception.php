...
class Except{
	function __construct($message){
		$ret = ob_get_contents();
		$conf = YAML::YAMLLoad(DIR.'/core/configs/config.yaml');
		if(isset($conf['status']) and strtolower($conf['status']) == 'debug'){
			ob_end_clean();	
			...
			die(hEtml::fromFile(DIR.'/system/Exceptions.html', array(
			'line' => $ret,
			'where'=>$wh,
			'message'=>$message->getMessage(),
			'title'=>$title)));
		}else{
			die(hEtml::fromFile(DIR.'/system/404.html'));
		}
	}
}
?>