...
class YAML {
	...
	function YAMLLoadString($input) {
		$YAML = new YAML;
		return $YAML -> __loadString($input);
	}
...
do {
	if (!YAML_FROM_COMMAND_LINE)
		break;
	if (empty($_SERVER['argc']) || $_SERVER['argc'] < 2)
		break;
	if (empty($_SERVER['PHP_SELF']) || $_SERVER['PHP_SELF'] != 'YAML.php')
		break;
	$file = $argv[1];
	printf("YAML loading file: %s\n", $file);
	print_r(YAML_load_file($file));
} while (0);
?>